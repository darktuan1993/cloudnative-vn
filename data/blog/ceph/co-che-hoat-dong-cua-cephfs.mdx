...
1. Client gửi request ghi file

- Gửi request ghi /docs/report.pdf tới CephFS mountpoint.
- Giao tiếp với MDS (Metadata Server) để:
- Tạo entry metadata cho file mới.
- Gán inode, quyền truy cập, đường dẫn.

2. File được chia nhỏ thành các block

- CephFS sử dụng cơ chế lưu dữ liệu của RADOS, nên file sẽ được:
- Chia thành các chunk nhỏ (thường 4MB mỗi block – configurable)
- Gán Object ID cho từng block.
- Lưu trữ phân tán lên các OSD khác nhau.

- Ví dụ: report.pdf (100 MB) → chia thành 25 block → mỗi block lưu trên 1 OSD, hoặc nhiều bản sao.

3. Replication (tự sao lưu)

- Mỗi block được replica (mặc định 3 bản) để đảm bảo an toàn dữ liệu.
- Ví dụ:

Block 1 → OSD1, OSD3, OSD5

Block 2 → OSD2, OSD4, OSD6

4. MDS cập nhật metadata
   Lưu thông tin:
   report.pdf gồm 25 block, các block nằm ở đâu, block nào thuộc OSD nào.
...
[Client]
   ...
   |
 [MDS] -- Gán inode, metadata
   |
   v
Chia file PDF thành các block nhỏ (4MB, 8MB...)
   |
   v
+--------+  +--------+  +--------+
...
+--------+  +--------+  +--------+
   |           |           |
   v           v           v
[OSD1]     [OSD2]      [OSD3]     (và các bản sao ở OSD4,5,6...)
